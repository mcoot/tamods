sqHud.rabbit_leaderboard = function(x, y)
	...
	for i = 0,2 do
		...
		if name ~= "" then
			y = y + 28 * i
			drawRect(x, y, x + 260, y + 26, sqHud.bg_color1)
			-- Different color if this is us
			if name == my_name then
				color = sqHud.team_colors_text[1]
				inTop3 = true
			end
	...
	if not inTop3 then
		y = y + 28
		drawRect(x, y, x + 260, y + 26, sqHud.bg_color1)
		drawText(sqHud.my_rank .. ". " .. player.name(), sqHud.team_colors_text[1], x + 5, y + 14, 0, 1)
		drawText(player.score(), sqHud.team_colors_text[1], x + 255, y + 14, 2, 1)
	end
end

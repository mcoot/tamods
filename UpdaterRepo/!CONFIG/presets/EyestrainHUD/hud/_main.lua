...
require(preset .. "hud/widgets/speedometer")
require(preset .. "hud/widgets/regen_timer")
...
function onDrawCustomHud(res_x, res_y)
	...
	if chat_vgs then chat_vgs(20, res_y - 20) end
	if kill_feed then kill_feed(res_x - 20, res_y - 40) end
	...
	if game_type == "TrGame_TRCTF" or game_type == "TrGame_TrCTFBlitz" or game_type == "TrGame_TRTeamRabbit" then
		...
		if flag_status then flag_status(53, 75) end
	...
	if player.isAlive() then
		...
		if player.hasFlag() then
			drawRect(center_x - 40, center_y + 200, center_x + 40, center_y + 225, rgba(255, 0, 0, 150))
			drawSmallText("F L A G", rgb(255, 255, 255), center_x, center_y + 212, 1, 1, 1)
		end
		-- Draw a message if we have rage active
		if player.isRaged() then
			drawRect(center_x - 40, center_y + 235, center_x + 40, center_y + 260, rgba(255, 0, 0, 150))
			drawSmallText("R A G E", rgb(255, 255, 255), center_x, center_y + 247, 1, 1, 1)
		end
		...
		if respawn_time > 0 then
			drawSmallText(respawn_time, text_color1, center_x, res_y * 0.9, 1, 1, 1)
		end
	end
end

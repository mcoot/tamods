...
module.name               = "Basic_Number_Energy"  -- This HAS to be the same as the filename minus the trailing .lua
module.opts               = {}
...
function module.draw(res_x, res_y)
	...
	local val = math.floor(player.energy())

	local color
	if module.opts.Dynamic_Color then
		color = lerpColor(module.opts.Low, module.opts.Full, player.energyPct())
	else

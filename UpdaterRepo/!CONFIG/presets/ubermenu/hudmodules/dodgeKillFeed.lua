...
module.name                  = "dodgeKillFeed"  -- This HAS to be the same as the filename minus the trailing .lua
module.opts                  = {}
module.opts.X_Position       = 1.0
module.opts.Y_Position       = 47.0
module.opts.Our_Name         = rgb(255,255,0)
module.opts.Background       = rgba(0,0,0,0)
module.opts.Message_Amount   = 16
module.opts.Message_Lifetime = 5
module.opts.Feed_Small       = false
...
function module.draw(res_x, res_y)
	...
	while i <= #messages do
		...
		if game.realTimeSeconds() - item.time >= module.opts.Message_Lifetime or game.realTimeSeconds() - module.opts.Message_Lifetime < 0 then
			...
			if module.opts.Feed_Small == true then			
				...
				textsize = getTextSize(item.killer, 1)
				drawRect(xpos, y_pos, x_pos + textsize.x, y_pos + 20, module.opts.Background)
				drawText(item.killer, item.kcolor, x_pos, y_pos + 11, 0, 1)
			end
			...
			if module.opts.Feed_Small == true then
				...
				textsize = getTextSize(item.killtype, 1)
				drawRect(x_pos, y_pos, x_pos + textsize.x, y_pos + 20, module.opts.Background)
				drawText(item.killtype, rgb(160,160,160), x_pos, y_pos + 11, 0, 1)
			end
			...
			if module.opts.Feed_Small == true then
				...
				textsize = getTextSize(item.victim, 1)
				drawRect(x_pos, y_pos, x_pos + textsize.x + 8, y_pos + 20, module.opts.Background)
				drawText(item.victim, item.vcolor, x_pos, y_pos + 11, 0, 1)
			end

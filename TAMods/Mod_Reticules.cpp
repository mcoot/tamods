...
bool TrPlayerPawn_Tick(int ID, UObject *dwCallingObject, UFunction* pFunction, void* pParams, void* pResult)
{
	...
	if (hud && hud->m_GFxHud && hud->m_GFxHud->Reticules)
	{
		...
		{
			wchar_t buff[128];
			if (g_config.shouldReloadCrosshairs)
			{
				...
				{
					wsprintf(buff, L"_root.reticulesMC.reticules.reticule_%02d_mc.hit", reti);
					UGFxObject *obj = ((UGFxMoviePlayer *)reticule->Outer)->GetVariableObject(buff, NULL);
					if (obj)
					{
						obj->SetFloat(L"_y", offset / g_config.crosshairScale);
						obj->SetFloat(L"_xscale", (1.0f / g_config.crosshairScale) * 100.0f);
						obj->SetFloat(L"_yscale", (1.0f / g_config.crosshairScale) * 100.0f);
					}
				}
				...
				g_config.shouldReloadCrosshairs = false;
			}

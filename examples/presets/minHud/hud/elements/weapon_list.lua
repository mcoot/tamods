...
function weapon_list(x, y)
	...
	for i, wep in ipairs(weapons) do
		...
		if not ((wep == 6 and weapon.isPack(6)) or weapon.ammo(wep) == -1) then
			...
			-- Try to prevent super wide boxes
			local nameTextSize = 1
			local myWidth = math.max(getSmallTextSize(get_weapon_name_abbr(weapon.name(wep)), nameTextSize).x + 4, boxWidth)
			-- if myWidth > 120 then
				-- nameTextSize = 0.75
				-- myWidth = math.max(getSmallTextSize(weapon.name(wep), nameTextSize).x + 4, boxWidth)
			-- end
			...
			drawSmallText(get_weapon_name_abbr(weapon.name(wep)), rgba(255, 255, 255, 200), x_pos + myWidth/2, y_pos + 8, 1, 0, nameTextSize) --wep == 6 and "Pack" or wep == 7 and "Belt" or i

function generator_status(x, y)
	...
	if not game.isGenUp(left_team) then
		...
		if repair_timer > 0 then
			...
			drawSmallText(repair_timer, team_colors_text[left_color], x - 99, y + 50, 1, 1, 1)
		else
			-- Auto repairing is turned off, just display that the generator is down
			drawSmallText("No Power", team_colors_text[left_color], x - 99, y + 50, 1, 1, 1)
		end
	end
	-- Right
	if not game.isGenUp(right_team) then
		...
		if repair_timer > 0 then
			-- Display the remaining time until auto repair
			drawSmallText(repair_timer, team_colors_text[right_color], x + 99, y + 50, 1, 1, 1)
		else
			-- Auto repairing is turned off, just display that the generator is down
			drawSmallText("No Power", team_colors_text[right_color], x + 99, y + 50, 1, 1, 1)
		end
	end
end
